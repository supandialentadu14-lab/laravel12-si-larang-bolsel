...
<div x-data="{
      ...
      <form action="{{ route('categories.index') }}" method="GET" class="space-y-4">
        <div class="space-y-1.5">
          ...
          <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari jenis belanja..." class="w-full pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-xs font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors">
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 pt-2">
          ...
          <a href="{{ route('categories.index') }}" class="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center transition-colors">
            Reset
          </a>
        </div>
      </form>
  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between px-2">
      ...
      <span class="text-[9px] font-bold text-slate-300 uppercase tracking-widest transition-colors">{{ $categories->count() }} Kategori</span>
    </div>
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between">
            <div>
              <h3 class="text-sm font-black text-slate-800 uppercase tracking-tight truncate leading-tight transition-colors">{{ $category->name }}</h3>
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 line-clamp-1 transition-colors">{{ $category->description ?: 'Tidak ada keterangan' }}</p>
            </div>
          </div>
