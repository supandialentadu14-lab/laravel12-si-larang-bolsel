...
<div class="space-y-6">
    <div class="flex items-center justify-between px-2">
        <div>
            <h1 class="text-2xl font-black text-slate-800 dark:text-white transition-colors uppercase tracking-tight">Tambah Barang</h1>
            <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mt-1">Input Data Barang Baru</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 border border-slate-50 dark:border-slate-800 shadow-sm transition-colors">
        <form action="{{ route('products.store') }}" method="POST" class="space-y-5">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors" required>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" value="{{ old('sku', $newSku ?? '') }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-400 dark:text-slate-600 outline-none transition-colors" readonly>
                </div>
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="min_stock" min="0" value="{{ old('min_stock') }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="price" step="0.01" value="{{ old('price') }}" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors" required>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="description" rows="4" class="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors">{{ old('description') }}</textarea>
