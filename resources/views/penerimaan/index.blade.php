...
<div x-data="{
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('reports.penerimaan.form') }}" class="w-10 h-10 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-100 flex items-center justify-center active:scale-90 transition-transform">
  ...
  <div x-show="showFilters" x-collapse x-cloak>
    <div class="bg-white rounded-[2.5rem] p-6 border border-slate-50 shadow-sm space-y-4">
      ...
      <form action="{{ route('reports.penerimaan.list') }}" method="GET" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3 pt-2">
          ...
          <a href="{{ route('reports.penerimaan.list') }}" class="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center">Reset</a>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="bg-indigo-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-indigo-100 overflow-hidden relative group">
    ...
    <div class="relative z-10">
      ...
      <p class="text-[9px] font-bold mt-2 opacity-80 uppercase tracking-widest">Periode Anggaran {{ now()->year }}</p>
    </div>
  </div>
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between">
              <div class="pr-2">
                <a href="{{ route('reports.penerimaan.show', $row['id']) }}" class="block group/link">
            ...
            <div class="mt-4 space-y-2">
              <div class="flex items-center gap-2 text-slate-500">
                ...
                <a href="{{ route('reports.pemeriksaan.list', ['search' => $row['pemeriksaan_nomor']]) }}" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
                  {{ $row['pemeriksaan_nomor'] ?: '-' }}
                </a>
              </div>
              ...
              <div class="pt-2 border-t border-slate-50 space-y-2">
                ...
                <div class="flex items-center justify-end gap-1.5 flex-wrap">
                  <a href="{{ route('reports.penerimaan.show', $row['id']) }}" class="w-8 h-8 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                    <i class="fas fa-eye text-[10px]"></i>
                  </a>
                  <a href="{{ route('reports.penerimaan.edit', $row['id']) }}" class="w-8 h-8 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                    <i class="fas fa-edit text-[10px]"></i>
                  </a>
                  <form action="{{ route('reports.penerimaan.delete', $row['id']) }}" method="POST" class="inline">
                    @csrf @method('DELETE')
