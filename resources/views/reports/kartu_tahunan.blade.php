...
@section('report_content')
    {{-- Header shown once for the whole report --}}
    <div class="text-center mb-4 pb-2">
        ...
        <h5 class="text-xs font-semibold mt-1">Per Periode
            ...
            {{ \Carbon\Carbon::parse($endDate)->translatedFormat('d F Y') }}</h5>
    </div>
    ...
    @forelse($grouped as $productId => $data)
        <div class="product-section mb-4 {{ !$loop->last ? 'border-b border-dashed border-gray-200 pb-3' : '' }}">
            <table class="info-table mb-2">
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td><strong>{{ $data['product']->satuan ?? '-' }}</strong></td>
                </tr>
            </table>
                ...
                <tbody>
                    ...
                    @foreach($data['rows'] as $row)
                        <tr align="center" style="font-size: 9px;">
                            ...
                            <td align="left" class="col-wrap">{{ $row['nosur'] ?? '-' }}</td>
                            <td align="left" class="uppercase font-semibold col-wrap">{{ $data['product']->name }}</td>
                            ...
                            <td align="right">
                                {{ $row['masuk'] ? number_format($row['masuk'] * ($row['harga'] ?? 0), 0, ',', '.') : '-' }}</td>
                            <td align="right">
                                {{ $row['keluar'] ? number_format($row['keluar'] * ($row['harga'] ?? 0), 0, ',', '.') : '-' }}</td>
                            <td align="right" class="font-bold">
                                {{ number_format($row['saldo'] * ($row['harga'] ?? 0), 0, ',', '.') }}</td>
                            <td></td>
                        </tr>
                    @endforeach
                    <tr class="text-[10px]">
                        <td colspan="6" align="center"><strong>Saldo Per
                                {{ \Carbon\Carbon::parse($endDate)->translatedFormat('d F Y') }}</strong></td>
                        <td align="center">
                            <strong>{{ $data['saldo_akhir'] == 0 ? 'Nihil' : number_format($data['saldo_akhir'], 0, ',', '.') }}</strong>
                        </td>
                        <td colspan="3"></td>
                        <td align="right">
                            <strong>{{ $data['saldo_akhir'] == 0 ? 'Nihil' : number_format($data['saldo_akhir'] * (count($data['rows']) > 0 ? ($data['rows'][count($data['rows']) - 1]['harga'] ?? 0) : 0), 0, ',', '.') }}</strong>
                        </td>
                        <td></td>
                    </tr>
                </tbody>
    ...
    {{-- Signatures shown once at the bottom --}}
