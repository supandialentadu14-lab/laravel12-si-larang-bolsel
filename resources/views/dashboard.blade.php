...
@section('actions')
  <form action="{{ route('dashboard') }}" method="GET" id="dateFilterForm"
    class="dash-filter-form flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm backdrop-blur-sm bg-white/80 transition-colors">
    ...
    <input type="date" id="dateInput" name="date" value="{{ request('date') ?? date('Y-m-d') }}"
      class="dash-filter-input bg-transparent border-none text-slate-800 font-black outline-none"
...
<div class="space-y-8 pb-10">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <div class="dash-card bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition">
      ...
      <div>
        ...
        <p class="text-2xl font-extrabold text-gray-800 transition-colors">{{ $inToday }} <span class="text-sm font-normal text-gray-400">item</span></p>
    ...
    <div class="dash-card bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition">
      ...
      <div>
        ...
        <p class="text-2xl font-extrabold text-gray-800 transition-colors">{{ $outToday }} <span class="text-sm font-normal text-gray-400">item</span></p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="dash-card lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-colors">
    ...
    <div class="dash-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-colors">
        ...
        @if($categoryValues->sum() > 0)
          <div class="relative mx-auto" style="width:180px; height:180px;">
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span class="text-2xl font-black text-gray-800 transition-colors">{{ $categoryValues->sum() }}</span>
              <span class="text-xs text-gray-400 transition-colors">Total</span>
            </div>
          </div>
          <div class="mt-4 space-y-2" id="categoryLegend"></div>
        @else
          <div class="flex flex-col items-center justify-center h-48 text-gray-300 transition-colors">
    ...
    <div class="dash-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-colors">
      ...
      <div class="p-4 space-y-3">
        ...
        <div class="flex items-center gap-3 p-2 rounded-xl border border-transparent hover:border-indigo-100 hover:bg-indigo-50/30 transition group">
          <div class="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-sm shrink-0">
            {{ $idx + 1 }}
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold text-gray-800 truncate group-hover:text-indigo-700 transition">{{ $p->name }}</p>
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ $p->category->name ?? 'Tanpa Kategori' }}</p>
          </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="dash-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-colors flex flex-col">
      <div class="flex items-center justify-between px-5 py-4 border-b border-gray-50 ">
        <h2 class="text-base font-extrabold text-gray-800 ">Aktivitas Terbaru</h2>
        <a href="{{ route('stock.index') }}"
          class="text-xs font-black text-indigo-500 hover:text-indigo-700  transition">Lihat Semua →</a>
      </div>
      <div class="flex-1 overflow-y-auto divide-y divide-gray-50 max-h-[280px] dash-scroll">
        ...
        <div class="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold text-gray-800 truncate leading-tight">{{ $tx->product?->name ?? '-' }}</p>
            <p class="text-[10px] text-gray-400 font-bold uppercase">{{ $tx->date->format('H:i') }}</p>
          </div>
        ...
        @empty
        <div class="flex flex-col items-center justify-center p-8 text-gray-300 transition-colors">
        ...
        legend.innerHTML += `
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-gray-600 font-bold">${label}</span>
            </div>
            <span class="text-xs font-black text-gray-800 transition-colors">${catValues[i]}</span>
          </div>`;
      });
    }
  }
