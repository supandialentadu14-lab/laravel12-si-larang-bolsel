...
<div x-data="notaPesanan()" class="bg-white rounded-lg shadow p-6 mb-6">
    ...
    <div x-show="showModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-start justify-center min-h-screen pt-10 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal" x-transition.scale.95 
                class="relative inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-slate-200">
                ...
                <form :action="isEdit ? '{{ url('reports/nota-pesanan') }}/' + editId + '/update' : '{{ route('reports.nota.save') }}'" 
                    ...
                    <div class="bg-slate-50 px-6 py-4 flex justify-between items-center border-t border-slate-200">
                        ...
                        <div class="flex gap-3">
                            <button type="submit" class="px-6 py-2 bg-indigo-600 border border-indigo-700 rounded-lg text-sm font-bold text-white hover:bg-indigo-700 transition drop-shadow-md flex items-center gap-2">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
        ...
        <div class="w-full lg:max-w-xl">
            <form action="{{ route('reports.nota.list') }}" method="GET" class="relative group">
                <div x-data="{ query: '{{ request('search') }}' }" class="relative">
                    ...
                    <span x-show="query" @click="location.href='{{ route('reports.nota.list') }}'" class="absolute inset-y-0 right-0 flex items-center pr-4 text-slate-300 cursor-pointer hover:text-rose-500 transition-colors">
