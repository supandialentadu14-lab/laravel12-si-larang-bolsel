...
<div x-data="{
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('reports.opname.form') }}" class="w-10 h-10 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-100 flex items-center justify-center active:scale-90 transition-transform">
  ...
  <div x-show="showFilters" x-collapse x-cloak>
    <div class="bg-white rounded-[2.5rem] p-6 border border-slate-50 shadow-sm space-y-4">
      ...
      <form action="{{ route('reports.opname.list') }}" method="GET" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3 pt-2">
          ...
          <a href="{{ route('reports.opname.list') }}" class="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center">Reset</a>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="bg-indigo-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-indigo-100 overflow-hidden relative group">
    ...
    <div class="relative z-10">
      ...
      <h2 class="text-3xl font-black mt-2 tracking-tight">{{ $items->total() }} Laporan</h2>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="mt-4 space-y-2">
            <div class="flex items-center gap-2 text-slate-500">
              ...
              <span class="text-[10px] font-medium truncate">{{ $item['tempat'] }}</span>
            </div>
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <span class="text-[9px] font-black text-slate-400 uppercase">Pelaksana:</span>
                <span class="text-[9px] font-bold text-slate-600 uppercase">{{ $item['pihak_kedua'] }}</span>
              </div>
              
              {{-- Actions --}}
              <div class="flex items-center gap-1.5">
                <a href="{{ route('reports.opname.show', $item['id']) }}" class="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <i class="fas fa-eye text-[10px]"></i>
                </a>
                <a href="{{ route('reports.opname.edit', $item['id']) }}" class="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <i class="fas fa-edit text-[10px]"></i>
                </a>
                <form action="{{ route('reports.opname.delete', $item['id']) }}" method="POST" class="inline">
                  @csrf @method('DELETE')
