...
<div x-show="mobileMenuOpen" data-mobile-sheet
    ...
    <!-- Panel (Bottom Sheet) -->
    <div class="absolute inset-x-0 bottom-0 max-h-[85vh] w-full bg-white dark:bg-slate-900 rounded-t-[2.5rem] shadow-2xl flex flex-col transition-colors duration-300"
        ...
        <!-- Menu Header -->
        <div class="px-8 pb-6 border-b border-gray-50 dark:border-slate-800 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-black">
                    <i class="fas fa-grid-2"></i>
                </div>
                <div>
                    <h3 class="font-black text-sm tracking-widest text-gray-900 dark:text-white uppercase">MENU NAVIGASI</h3>
                    <p class="text-[9px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Semua Akses Halaman</p>
                </div>
            </div>
            <button @click="mobileMenuOpen = false" class="btn-icon-mini bg-gray-50 dark:bg-slate-800 text-gray-400 dark:text-slate-500">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-6 space-y-2 no-scrollbar pb-24">
            ...
            <div class="mb-6">
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ route('products.index', ['search' => $lp->name]) }}" class="flex items-center gap-4 px-4 py-4 rounded-2xl bg-red-50/50 dark:bg-red-950/20 border border-red-100/50 dark:border-red-900/30 transition active:scale-[0.98]">
                        <div class="w-10 h-10 rounded-xl bg-red-500 text-white flex items-center justify-center flex-shrink-0 shadow-sm shadow-red-100 dark:shadow-none">
                            <i class="fas fa-exclamation-triangle text-xs"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-[11px] font-black text-red-900 dark:text-red-200 truncate uppercase tracking-tight">{{ $lp->name }}</p>
                            <p class="text-[9px] font-bold text-red-600 dark:text-red-400 mt-1 uppercase tracking-widest">Tersisa: {{ $lp->stock }} {{ $lp->unit }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
            ...
            <div class="grid grid-cols-1 gap-2">
                <!-- Dashboard -->
                <a href="{{ route('dashboard') }}" class="flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('dashboard') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700' }}">
                ...
                <div x-data="{ open: {{ request()->routeIs('categories.*') || request()->routeIs('products.*') || request()->routeIs('suppliers.*') ? 'true' : 'false' }} }" class="space-y-2">
                    <button @click="open = !open" class="w-full flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('categories.*') || request()->routeIs('products.*') || request()->routeIs('suppliers.*') ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 shadow-inner' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                        <div class="flex items-center gap-4">
                    ...
                    <div x-show="open" x-collapse class="px-2 space-y-1">
                        <a href="{{ route('categories.index') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('categories.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('categories.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Kategori
                        </a>
                        <a href="{{ route('products.index') }}" class="flex items-center justify-between px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('products.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="flex items-center gap-4">
                                <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('products.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                                Barang
                            </div>
                            @if (isset($lowStockCount) && $lowStockCount > 0)
                        ...
                        <a href="{{ route('suppliers.index') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('suppliers.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('suppliers.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Penyedia
                        </a>
                    </div>
                </div>
                ...
                <div x-data="{ open: {{ request()->routeIs('stock.*') || request()->routeIs('reports.belanja.modal.list') ? 'true' : 'false' }} }" class="space-y-2">
                    <button @click="open = !open" class="w-full flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('stock.*') || request()->routeIs('reports.belanja.modal.list') ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 shadow-inner' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                        <div class="flex items-center gap-4">
                    ...
                    <div x-show="open" x-collapse class="px-2 space-y-1">
                        ...
                        <a href="{{ route('stock.index') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('stock.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('stock.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Mutasi Masuk/Keluar
                        </a>
                        @endif
                        @if(auth()->user()->hasPermission('laporan_belanja'))
                        <a href="{{ route('reports.belanja.modal.list') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('reports.belanja.modal.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('reports.belanja.modal.*') ? 'text-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                ...
                <div x-data="{ open: {{ request()->routeIs('reports.nota.list') || request()->routeIs('reports.pemeriksaan.list') || request()->routeIs('reports.penerimaan.list') || request()->routeIs('reports.kwitansi.list') ? 'true' : 'false' }} }" class="space-y-2">
                    <button @click="open = !open" class="w-full flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('reports.nota.list') || request()->routeIs('reports.pemeriksaan.list') || request()->routeIs('reports.penerimaan.list') || request()->routeIs('reports.kwitansi.list') ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 shadow-inner' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                        <div class="flex items-center gap-4">
                    ...
                    <div x-show="open" x-collapse class="px-2 space-y-1">
                        ...
                        <a href="{{ route('reports.nota.list') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('reports.nota.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('reports.nota.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Daftar Surat Pesanan
                        </a>
                        @endif
                        @if(auth()->user()->hasPermission('pemeriksaan'))
                        <a href="{{ route('reports.pemeriksaan.list') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('reports.pemeriksaan.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('reports.pemeriksaan.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Daftar Pemeriksaan
                        </a>
                        @endif
                        @if(auth()->user()->hasPermission('penerimaan'))
                        <a href="{{ route('reports.penerimaan.list') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('reports.penerimaan.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('reports.penerimaan.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Daftar Penerimaan
                        </a>
                        @endif
                        @if(auth()->user()->hasPermission('berkas_lainnya'))
                        <a href="{{ route('reports.kwitansi.list') }}" class="flex items-center gap-4 px-5 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest {{ request()->routeIs('reports.kwitansi.*') ? 'text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-800 shadow-sm' : 'text-gray-400 dark:text-slate-500' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('reports.kwitansi.*') ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-slate-700' }}"></div>
                            Daftar Kwitansi
                        </a>
                        @endif
                    </div>
                ...
                <div class="pt-8 pb-4">
                    <p class="px-5 text-[9px] font-black text-gray-300 dark:text-slate-600 uppercase tracking-[0.2em] mb-4">Pengaturan Sistem</p>
                    
                    <div class="space-y-2">
                        <a href="{{ route('settings.opd.edit') }}" class="flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('settings.opd.*') ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                            <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('users.index') }}" class="flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('users.*') ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                            <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('activity_log.index') }}" class="flex items-center justify-between px-5 py-4 rounded-2xl transition {{ request()->routeIs('activity_log.*') ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-300' }}">
                            <div class="flex items-center gap-4">
            ...
            <!-- Logout Section -->
            <div class="p-8 border-t border-gray-50 dark:border-slate-800 transition-colors">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-3 px-6 py-5 rounded-[1.5rem] bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 text-[10px] font-black uppercase tracking-[0.15em] transition active:scale-95 border border-transparent dark:border-rose-900/30">
...
<nav x-init="$nextTick(() => { const activeItem = $el.querySelector('.active-menu'); if (activeItem) activeItem.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); })"
    ...
    class="transition-all duration-500 ease-in-out transform fixed bottom-0 left-0 right-0 z-[9999] glass-card border-t border-gray-100/50 dark:border-slate-800/50 bottom-nav shadow-[0_-4px_20px_rgba(0,0,0,0.08)] dark:shadow-[0_-4px_20px_rgba(15,23,42,0.8)] lg:hidden">
    <div class="flex items-center justify-center min-w-full overflow-x-auto no-scrollbar py-4 scroll-smooth snap-x snap-mandatory">
        ...
        <a data-master-nav data-skip-transition href="{{ route('products.index') }}" @click.prevent="masterMenuOpen = true" class="flex flex-col items-center gap-1 shrink-0 flex-1 snap-center {{ request()->routeIs('products.*') || request()->routeIs('categories.*') || request()->routeIs('suppliers.*') ? 'active-menu scale-110' : '' }}">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 {{ request()->routeIs('products.*') || request()->routeIs('categories.*') || request()->routeIs('suppliers.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 dark:shadow-indigo-900/40' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500' }}">
                <i class="fas fa-layer-group text-lg"></i>
            </div>
            <span class="text-[8px] font-black uppercase tracking-widest whitespace-nowrap {{ request()->routeIs('products.*') || request()->routeIs('categories.*') || request()->routeIs('suppliers.*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600 font-bold' }}">Master</span>
        </a>
        @endif
        ...
        <a data-flow-nav data-skip-transition href="{{ route('stock.index') }}" @click.prevent="flowMenuOpen = true" class="flex flex-col items-center gap-1 shrink-0 flex-1 snap-center {{ request()->routeIs('stock.*') || request()->routeIs('reports.belanja.modal.*') || request()->routeIs('reports.nota.*') || request()->routeIs('reports.pemeriksaan.*') || request()->routeIs('reports.penerimaan.*') || request()->routeIs('reports.kwitansi.*') || request()->routeIs('reports.opname.*') || request()->routeIs('reports.pinjam.*') ? 'active-menu scale-110' : '' }}">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 {{ request()->routeIs('stock.*') || request()->routeIs('reports.belanja.modal.*') || request()->routeIs('reports.nota.*') || request()->routeIs('reports.pemeriksaan.*') || request()->routeIs('reports.penerimaan.*') || request()->routeIs('reports.kwitansi.*') || request()->routeIs('reports.opname.*') || request()->routeIs('reports.pinjam.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 dark:shadow-indigo-900/40' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500' }}">
                <i class="fas fa-folder-open text-lg"></i>
            </div>
            <span class="text-[8px] font-black uppercase tracking-widest whitespace-nowrap {{ request()->routeIs('stock.*') || request()->routeIs('reports.belanja.modal.*') || request()->routeIs('reports.nota.*') || request()->routeIs('reports.pemeriksaan.*') || request()->routeIs('reports.penerimaan.*') || request()->routeIs('reports.kwitansi.*') || request()->routeIs('reports.opname.*') || request()->routeIs('reports.pinjam.*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600 font-bold' }}">Berkas</span>
        </a>
        @endif
        ...
        <a href="{{ route('dashboard') }}" class="flex flex-col items-center gap-1 shrink-0 flex-1 snap-center {{ request()->routeIs('dashboard') ? 'active-menu scale-110' : '' }}">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 {{ request()->routeIs('dashboard') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 dark:shadow-indigo-900/40' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-slate-700' }}">
                <i class="fas fa-home text-lg"></i>
            </div>
            <span class="text-[8px] font-black uppercase tracking-widest whitespace-nowrap {{ request()->routeIs('dashboard') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600 font-bold' }}">Beranda</span>
        </a>
        ...
        <a data-settings-nav data-skip-transition href="{{ route('settings.opd.edit') }}" @click.prevent="settingsMenuOpen = true" class="flex flex-col items-center gap-1 shrink-0 flex-1 snap-center {{ request()->routeIs('settings.opd.*') || request()->routeIs('users.*') || request()->routeIs('activity_log.*') ? 'active-menu scale-110' : '' }}">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 {{ request()->routeIs('settings.opd.*') || request()->routeIs('users.*') || request()->routeIs('activity_log.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 dark:shadow-indigo-900/40' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500' }}">
                <i class="fas fa-sliders text-lg"></i>
            </div>
            <span class="text-[8px] font-black uppercase tracking-widest whitespace-nowrap {{ request()->routeIs('settings.opd.*') || request()->routeIs('users.*') || request()->routeIs('activity_log.*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600 font-bold' }}">Manajemen</span>
        </a>
        ...
        <a href="{{ route('profile.edit') }}" class="flex flex-col items-center gap-1 shrink-0 flex-1 snap-center {{ request()->routeIs('profile.*') ? 'active-menu scale-110' : '' }}">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 {{ request()->routeIs('profile.*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 dark:shadow-indigo-900/40' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500' }}">
                <i class="fas fa-user-circle text-lg"></i>
            </div>
            <span class="text-[8px] font-black uppercase tracking-widest whitespace-nowrap {{ request()->routeIs('profile.*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600 font-bold' }}">Profil</span>
        </a>
    </div>
</nav>
