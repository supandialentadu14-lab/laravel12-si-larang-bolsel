...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    @forelse ($items as $item)
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            {{-- Header --}}
            <div class="px-5 sm:px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-white">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
                    <div class="flex items-center gap-4 w-full sm:w-auto">
                        ...
                        <div class="flex-1 min-w-0">
                            <h2 class="text-base sm:text-lg font-bold text-slate-800 uppercase tracking-tight leading-snug break-words">{{ $item->nama_opd }}</h2>
                            @if($item->singkatan_opd)
                                <div class="mt-1.5">
                                    <span class="bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-widest inline-block">{{ $item->singkatan_opd }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                {{-- Alamat --}}
                <div class="mt-4 bg-white/50 p-3 sm:p-4 rounded-xl border border-slate-200/60 flex items-start gap-3 shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)]">
                    <i class="fas fa-map-marker-alt text-slate-400 mt-0.5 shrink-0"></i>
                    <p class="text-[11px] sm:text-xs text-slate-600 leading-relaxed font-medium">{{ $item->alamat_opd }}</p>
                </div>
            </div>

            {{-- Info banner --}}
            <div class="px-6 py-3 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-slate-500">
                <div class="flex items-center gap-2">
                    <i class="fas fa-calendar-alt text-slate-400"></i>
                    <span>Tutup Buku: <strong>{{ $item->tutup_buku_date ? \Carbon\Carbon::parse($item->tutup_buku_date)->translatedFormat('d M Y') : 'Belum diatur' }}</strong></span>
                </div>
                <div class="flex items-center gap-2 mt-1 sm:mt-0">
                    <i class="fas fa-clock text-slate-400"></i>
                    <span>Terakhir diupdate: <strong>{{ $item->updated_at?->translatedFormat('d M Y H:i') }}</strong></span>
                </div>
            </div>
    ...
    @empty
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden p-12 text-center">
            <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <i class="fas fa-building text-3xl text-slate-400"></i>
            </div>
            <h3 class="text-lg font-bold text-slate-800 mb-2">Data OPD Belum Diatur</h3>
            <p class="text-slate-500 max-w-md mx-auto mb-6">Silakan atur identitas dan pejabat OPD untuk keperluan prefilling dokumen laporan, kwitansi, dan nota pesanan.</p>
