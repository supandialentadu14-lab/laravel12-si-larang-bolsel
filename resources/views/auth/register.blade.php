<x-guest-layout title="DAFTAR" subtitle="Buat akun baru untuk mulai mengelola aset">
  ...
  <form method="POST" action="{{ route('register') }}" class="space-y-2 sm:space-y-3" x-data="{ show: false }">
      ...
      <div class="space-y-0.5 sm:space-y-1">
        ...
        <div class="relative group">
          ...
          <input id="password" :type="show ? 'text' : 'password'" name="password" required
            class="w-full pl-10 sm:pl-12 pr-10 sm:pr-12 py-2 sm:py-2.5 bg-slate-50 border-2 border-transparent focus:border-indigo-600 focus:bg-white rounded-[1.2rem] text-[10px] sm:text-[11px] font-bold text-slate-800 transition-all outline-none"
            placeholder="••••••••">
    ...
    {{-- Action --}}
    <button type="submit" class="w-full py-3 sm:py-3.5 bg-indigo-600 text-white rounded-[1.5rem] sm:rounded-[1.8rem] text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-indigo-100 hover:bg-slate-800 transition-all mt-1 sm:mt-2">
