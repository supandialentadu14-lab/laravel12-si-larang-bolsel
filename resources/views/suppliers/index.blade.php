...
<div x-data="{
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('suppliers.create') }}" class="w-10 h-10 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-100 flex items-center justify-center active:scale-90 transition-transform">
      ...
      <form action="{{ route('suppliers.index') }}" method="GET" class="space-y-4">
        <div class="space-y-1.5">
          ...
          <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari penyedia..." class="w-full pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-xs font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors">
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 pt-2">
          ...
          <a href="{{ route('suppliers.index') }}" class="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center transition-colors">
            Reset
          </a>
        </div>
      </form>
  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between px-2">
      ...
      <span class="text-[9px] font-bold text-slate-300 uppercase tracking-widest transition-colors">{{ $suppliers->count() }} Ditampilkan</span>
    </div>
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-start justify-between">
            <div>
              <h3 class="text-sm font-black text-slate-800 uppercase tracking-tight truncate leading-tight transition-colors">{{ $supplier->name }}</h3>
              <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 transition-colors">Dir: {{ $supplier->dir ?: '-' }}</p>
            </div>
          </div>
          ...
          <div class="mt-4 space-y-2">
            ...
            <div class="flex items-center justify-between">
              <span class="text-[9px] font-mono bg-slate-50 px-2 py-1 rounded-lg text-slate-400 border border-slate-100 uppercase transition-colors">NPWP: {{ $supplier->npwp }}</span>
              
              {{-- Quick Actions --}}
              <div class="flex items-center gap-1.5">
                <a href="{{ route('suppliers.edit', $supplier->id) }}" class="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
