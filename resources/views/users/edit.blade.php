...
<div class="space-y-6 pb-24 {{ !($isMobile ?? false) ? 'pt-10' : '' }}">
  ...
  <form action="{{ request()->routeIs('profile.edit') ? route('profile.update') : route('users.update', $user) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    {{-- Foto Profil --}}
    <div x-data="{ 
        imageUrl: '{{ $user->avatar ? asset('media/'.$user->avatar . '?v=' . ($user->avatar_updated_at?->timestamp ?? time())) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&background=ffffff&color=4F46E5' }}',
        fileChosen(event) {
