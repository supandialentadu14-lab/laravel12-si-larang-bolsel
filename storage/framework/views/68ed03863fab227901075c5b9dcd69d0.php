...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2 md:px-0">
        ...
        <form action="<?php echo e(route('backups.create')); ?>" method="POST" class="w-full">
            <?php echo csrf_field(); ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-app-surface p-6 rounded-[32px] border border-app-main shadow-sm relative overflow-hidden group transition-colors">
            <div class="relative z-10">
                ...
                <h3 class="text-3xl font-black text-app-main mt-1"><?php echo e(count($backups)); ?></h3>
        ...
        <div class="bg-app-surface p-6 rounded-[32px] border border-app-main shadow-sm relative overflow-hidden group transition-colors">
            <div class="relative z-10">
                ...
                <h3 class="text-xl font-black text-app-main mt-1"><?php echo e(count($backups) > 0 ? $backups[0]['at'] : 'Belum ada'); ?></h3>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-app-surface p-5 rounded-[2.5rem] border border-app-main shadow-sm transition-colors">
            ...
            <h3 class="text-2xl font-black text-app-main mt-2 leading-none"><?php echo e(count($backups)); ?></h3>
        ...
        <div class="bg-indigo-600 p-5 rounded-[2.5rem] shadow-lg shadow-indigo-600/20">
            ...
            <h3 class="text-xs font-black text-white mt-2 leading-tight"><?php echo e(count($backups) > 0 ? date('d M Y', strtotime($backups[0]['at'])) : 'N/A'); ?></h3>
            <div class="mt-3 flex items-center justify-between">
                <span class="text-[8px] font-bold text-indigo-200 uppercase tracking-tighter"><?php echo e(count($backups) > 0 ? date('H:i', strtotime($backups[0]['at'])) : ''); ?></span>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4">
            <?php $__empty_1 = true; $__currentLoopData = $backups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                ...
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0 pr-2">
                            <h4 class="text-xs font-black text-app-main tracking-tight break-all uppercase leading-snug"><?php echo e($file['name']); ?></h4>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-md text-[9px] font-black uppercase"><?php echo e($file['size']); ?></span>
                                <span class="text-[9px] font-bold text-app-muted uppercase tracking-tighter"><?php echo e(date('d/m/Y H:i', strtotime($file['at']))); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3 pt-4 border-t border-app-main/10">
                    <a href="<?php echo e(route('backups.download', $file['name'])); ?>" class="flex items-center justify-center gap-2 py-3 bg-slate-100 dark:bg-white/5 text-app-main rounded-2xl text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all">
                        <i class="fas fa-download text-[8px] text-indigo-600"></i>
                        Unduh
                    </a>
                    <form action="<?php echo e(route('backups.destroy', $file['name'])); ?>" method="POST" onsubmit="return confirm('Hapus file cadangan ini?')" class="w-full">
                        <?php echo csrf_field(); ?>
                    ...
                    <tr class="hover:bg-app-bg/50 transition-colors group">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-bold text-app-main tracking-tight uppercase leading-tight"><?php echo e($file['name']); ?></p>
                        ...
                        <td class="px-8 py-5">
                            <div class="flex items-center justify-end gap-2">
                                <a href="<?php echo e(route('backups.download', $file['name'])); ?>" class="btn-icon-mini text-app-muted hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-app-surface hover:shadow-sm transition-all" title="Unduh">
                                    <i class="fas fa-download"></i>
                                </a>
                                <form action="<?php echo e(route('backups.destroy', $file['name'])); ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus file cadangan ini?')">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
...
<?php echo $__env->make(($isMobile ?? false) ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/backups/index.blade.php ENDPATH**/ ?>