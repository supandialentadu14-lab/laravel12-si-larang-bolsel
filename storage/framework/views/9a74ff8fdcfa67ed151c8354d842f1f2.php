<?php $__env->startSection('content'); ?>
  <div class="space-y-6 pb-24">
    ...
    <div class="flex items-center justify-between">
      ...
      <a href="<?php echo e(route('reports.penerimaan.list')); ?>" class="w-10 h-10 rounded-2xl bg-app-surface border border-app-main shadow-sm flex items-center justify-center text-app-muted">
        <i class="fas fa-times text-xs"></i>
      </a>
    </div>

    <form action="<?php echo e(route('reports.penerimaan.save')); ?>" method="POST" class="space-y-6">
      <?php echo csrf_field(); ?>
      <input type="hidden" name="id" value="<?php echo e(session('penerimaan_current_id')); ?>">
          ...
          <div class="space-y-1.5">
            ...
            <select name="pemeriksaan_nomor" 
              ...
              <?php $__currentLoopData = $docs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($n['nomor']); ?>" <?php echo e((old('pemeriksaan_nomor', ($data['pemeriksaan_nomor'] ?? '') ?: (request('pemeriksaan_nomor') ?? ''))) == $n['nomor'] ? 'selected' : ''); ?>>
                  <?php echo e($n['nomor']); ?> • <?php echo e(\Carbon\Carbon::parse($n['tanggal'] ?? now())->translatedFormat('d F Y')); ?>

                </option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            <?php $__errorArgs = ['pemeriksaan_nomor'];
...
unset($__errorArgs, $__bag); ?>
          ...
          <div class="space-y-1.5">
            ...
            <textarea name="catatan" rows="2" class="w-full px-6 py-4 bg-app-bg border-none rounded-2xl text-xs font-bold text-app-main focus:ring-2 focus:ring-indigo-500/20 outline-none leading-relaxed" placeholder="Masukkan catatan jika diperlukan..."><?php echo e(old('catatan', $data['catatan'] ?? '')); ?></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-3 px-2">
        <a href="<?php echo e(route('reports.penerimaan.list')); ?>" class="flex-1 py-5 bg-app-surface text-app-muted border border-app-main rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] text-center">Batal</a>
        <button type="submit" class="flex-[2] py-5 bg-indigo-600 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 active:scale-95 transition-all">Simpan BASTB</button>
      </div>
    </form>
  </div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make($isMobile ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/penerimaan/create.blade.php ENDPATH**/ ?>