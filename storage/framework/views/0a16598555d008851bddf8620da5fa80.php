...
<div x-data="{
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-2">
      ...
      <a href="<?php echo e(route('reports.pinjam.form')); ?>" class="w-10 h-10 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-100 flex items-center justify-center active:scale-90 transition-transform">
  ...
  <div x-show="showFilters" x-collapse x-cloak>
    <div class="bg-white rounded-[2.5rem] p-6 border border-slate-50 shadow-sm space-y-4">
      ...
      <form action="<?php echo e(route('reports.pinjam.list')); ?>" method="GET" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3 pt-2">
          ...
          <a href="<?php echo e(route('reports.pinjam.list')); ?>" class="w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center">Reset</a>
        </div>
      </form>
    </div>
  </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="mt-4 space-y-2">
            <div class="flex items-center gap-2 text-slate-500">
              ...
              <span class="text-[10px] font-medium truncate"><?php echo e($item['pihak_kedua']); ?></span>
            </div>
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <span class="text-[9px] font-black text-slate-400 uppercase">Lokasi:</span>
                <span class="text-[9px] font-bold text-slate-600 uppercase"><?php echo e($item['tempat']); ?></span>
              </div>
              
              
              <div class="flex items-center gap-1.5">
                <a href="<?php echo e(route('reports.pinjam.show', $item['id'])); ?>" class="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <i class="fas fa-eye text-[10px]"></i>
                </a>
                <a href="<?php echo e(route('reports.pinjam.edit', $item['id'])); ?>" class="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                  <i class="fas fa-edit text-[10px]"></i>
                </a>
                <form action="<?php echo e(route('reports.pinjam.delete', $item['id'])); ?>" method="POST" class="inline">
...
<?php echo $__env->make($isMobile ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/pinjam_pakai/index.blade.php ENDPATH**/ ?>