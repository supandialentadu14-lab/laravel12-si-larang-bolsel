...
<div class="space-y-6 max-w-2xl mx-auto pb-20">
  ...
  <div class="flex items-center justify-between px-2">
    <div>
      <h1 class="text-2xl font-black text-slate-800 transition-colors uppercase tracking-tight">Impor Barang</h1>
      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Update Inventaris via Excel / CSV</p>
    </div>
    <a href="<?php echo e(route('products.index')); ?>" class="w-10 h-10 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center justify-center text-slate-400 active:scale-90 transition-all">
  ...
  <div class="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-xl shadow-indigo-100 overflow-hidden relative group">
    ...
    <div class="relative z-10">
      ...
      <div class="overflow-hidden rounded-2xl border border-white/20 bg-white/10 shadow-inner">
        <table class="w-full text-left border-collapse bg-white transition-colors">
          <thead>
          ...
          <tbody>
            <tr>
              <td class="border border-slate-100 bg-slate-50 text-center text-[9px] font-bold text-slate-400 py-3 transition-colors">1</td>
              <td class="border border-slate-100 px-4 py-3">
                <div class="space-y-1">
                  <span class="text-[10px] font-black text-slate-700 uppercase block tracking-tight transition-colors">Pensil 2B</span>
                  <span class="text-[8px] font-medium text-indigo-500 uppercase block tracking-widest opacity-60">Nama Barang</span>
                </div>
              </td>
              <td class="border border-slate-100 px-4 py-3">
                <div class="space-y-1">
                  <span class="text-[10px] font-black text-slate-700 uppercase block tracking-tight transition-colors">Alat Tulis</span>
                  <span class="text-[8px] font-medium text-indigo-500 uppercase block tracking-widest opacity-60">Kategori</span>
                </div>
              </td>
              <td class="border border-slate-100 px-4 py-3">
                <div class="space-y-1">
                  <span class="text-[10px] font-black text-slate-700 uppercase block tracking-tight transition-colors">2500</span>
                  <span class="text-[8px] font-medium text-indigo-500 uppercase block tracking-widest opacity-60">Harga</span>
                </div>
              </td>
            </tr>
            <tr>
              <td class="border border-slate-100 bg-slate-50 text-center text-[9px] font-bold text-slate-400 py-3 transition-colors">2</td>
              <td class="border border-slate-100 px-4 py-3 transition-colors">
                <span class="text-[10px] font-medium text-slate-400 italic">... data berikutnya</span>
              </td>
              <td class="border border-slate-100 px-4 py-3 text-center transition-colors">
  ...
  <div class="bg-white rounded-[2.5rem] p-8 border border-slate-50 shadow-sm transition-colors" x-data="{ fileName: '', isSelected: false }">
    <form action="<?php echo e(route('import.products')); ?>" method="POST" enctype="multipart/form-data" class="no-soft space-y-8">
      <?php echo csrf_field(); ?>
      ...
      <div class="space-y-4">
        ...
        <div class="relative group">
          ...
          <div class="border-2 border-dashed rounded-[2rem] p-12 text-center transition-all duration-300" 
            ...
            <p class="text-[8px] font-bold text-slate-300 mt-4 uppercase tracking-[0.2em] transition-colors">Maksimal 2MB</p>
          </div>
        </div>
...
$message = $__bag->first($__errorArgs[0]); ?>
          <div class="flex items-center gap-2 px-4 py-3 rounded-2xl bg-rose-50 text-rose-500 border border-rose-100">
            ...
            <p class="text-[10px] font-black uppercase tracking-widest"><?php echo e($message); ?></p>
          </div>
        <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
      ...
      <div class="grid grid-cols-2 gap-4 pt-4">
        <a href="<?php echo e(route('products.index')); ?>" class="w-full py-5 bg-slate-50 text-slate-400 rounded-3xl text-[10px] font-black uppercase tracking-widest text-center transition-all hover:bg-slate-100 hover:text-slate-600 ">
          Batal
        </a>
...
<?php echo $__env->make(($isMobile ?? false) ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/import/index.blade.php ENDPATH**/ ?>