<?php $__env->startSection('content'); ?>
  ...
  <div class="space-y-6 pb-24">
    ...
    <div class="flex items-center justify-between">
      ...
      <a href="<?php echo e(route('reports.opname.list')); ?>" class="w-10 h-10 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center justify-center text-slate-400">
        <i class="fas fa-times text-xs"></i>
      </a>
    </div>

    <form method="POST" action="<?php echo e(route('reports.opname.save')); ?>" x-data="opnameForm()" x-init="$nextTick(() => updatePembuka())" class="space-y-6">
      <?php echo csrf_field(); ?>
      <?php if(session('opname_current_id') || isset($data['id'])): ?>
        <input type="hidden" name="id" value="<?php echo e(session('opname_current_id') ?? $data['id']); ?>">
      <?php endif; ?>
      ...
      <div class="bg-white rounded-[2.5rem] p-6 border border-slate-50 shadow-sm space-y-6">
        ...
        <div class="space-y-4">
          <div class="space-y-1.5">
            ...
            <input type="text" name="nomor" value="<?php echo e($data['nomor'] ?? ''); ?>" 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1.5">
              ...
              <input x-ref="tanggal" @change="updatePembuka()" type="date" name="tanggal" value="<?php echo e($data['tanggal'] ?? now()->toDateString()); ?>" 
            ...
            <div class="space-y-1.5">
              ...
              <input x-ref="tempat" @input="updatePembuka()" type="text" name="tempat" value="<?php echo e($data['tempat'] ?? ($opd->nama_opd ?? '')); ?>" 
          ...
          <div class="space-y-1.5">
            ...
            <textarea x-ref="pembuka" name="pembuka" rows="3" class="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-xs font-bold focus:ring-2 focus:ring-indigo-500/20 outline-none leading-relaxed"><?php echo e($data['pembuka'] ?? ''); ?></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-indigo-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-indigo-100 space-y-6">
          <div class="flex items-center justify-between">
            ...
            <button type="button" @click="$refs.pp_nama.value='<?php echo e($opd->kepala_nama); ?>'; $refs.pp_nip.value='<?php echo e($opd->kepala_nip); ?>'; $refs.pp_jabatan.value='<?php echo e($opd->kepala_jabatan); ?>';" class="text-[8px] font-black uppercase px-2 py-1 bg-white/20 rounded-lg">Auto</button>
          </div>
          <div class="space-y-4">
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Nama Lengkap</label>
              <input x-ref="pp_nama" type="text" name="pihak_pertama[nama]" value="<?php echo e($data['pihak_pertama']['nama'] ?? ''); ?>" placeholder="Nama Atasan" 
            ...
            <div class="space-y-1.5">
              ...
              <input x-ref="pp_nip" type="text" name="pihak_pertama[nip]" value="<?php echo e($data['pihak_pertama']['nip'] ?? ''); ?>" placeholder="NIP" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-mono placeholder:text-white/40 outline-none">
            </div>
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Jabatan</label>
              <input x-ref="pp_jabatan" type="text" name="pihak_pertama[jabatan]" value="<?php echo e($data['pihak_pertama']['jabatan'] ?? ''); ?>" placeholder="Jabatan" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-bold placeholder:text-white/40 outline-none">
            </div>
          </div>
        </div>
        ...
        <div class="bg-rose-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-rose-100 space-y-6">
          <div class="flex items-center justify-between">
            ...
            <button type="button" @click="$refs.pk_nama.value='<?php echo e($opd->pengurus_nama); ?>'; $refs.pk_nip.value='<?php echo e($opd->pengurus_nip); ?>'; $refs.pk_jabatan.value='<?php echo e($opd->pengurus_jabatan); ?>';" class="text-[8px] font-black uppercase px-2 py-1 bg-white/20 rounded-lg">Auto</button>
          </div>
          <div class="space-y-4">
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Nama Lengkap</label>
              <input x-ref="pk_nama" type="text" name="pihak_kedua[nama]" value="<?php echo e($data['pihak_kedua']['nama'] ?? ''); ?>" placeholder="Nama Pengurus" 
            ...
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">NIP</label>
              <input x-ref="pk_nip" type="text" name="pihak_kedua[nip]" value="<?php echo e($data['pihak_kedua']['nip'] ?? ''); ?>" placeholder="NIP" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-mono placeholder:text-white/40 outline-none">
            </div>
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Jabatan</label>
              <input x-ref="pk_jabatan" type="text" name="pihak_kedua[jabatan]" value="<?php echo e($data['pihak_kedua']['jabatan'] ?? ''); ?>" placeholder="Jabatan" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-bold placeholder:text-white/40 outline-none">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-3 px-2">
        <a href="<?php echo e(route('reports.opname.list')); ?>" class="flex-1 py-5 bg-slate-100 text-slate-400 rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] text-center">Batal</a>
...
<?php echo $__env->make($isMobile ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/opname/create.blade.php ENDPATH**/ ?>