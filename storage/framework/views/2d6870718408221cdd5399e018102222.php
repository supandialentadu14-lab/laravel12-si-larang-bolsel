<?php $__env->startSection('content'); ?>
  <script>
    window.formData = function () {
      return {
        items: <?php echo json_encode(($data['items'] ?? [['_key' => 1]])); ?>,
        nextKey: <?php echo e(count($data['items'] ?? []) + 2); ?>,
        ensureKeys() {
        ...
        prefill() { 
          this.items = <?php echo json_encode(($data['items'] ?? [])); ?>; 
  ...
  <div class="space-y-6 pb-24">
    ...
    <div class="flex items-center justify-between">
      ...
      <a href="<?php echo e(route('reports.pinjam.list')); ?>" class="w-10 h-10 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center justify-center text-slate-400">
        <i class="fas fa-times text-xs"></i>
      </a>
    </div>

    <form method="POST" action="<?php echo e(route('reports.pinjam.save')); ?>" x-data="formData()" x-init="init()" class="space-y-6">
      <?php echo csrf_field(); ?>
      <?php if(session('pinjam_pakai_current_id') || isset($data['id'])): ?>
        <input type="hidden" name="id" value="<?php echo e(session('pinjam_pakai_current_id') ?? $data['id']); ?>">
      <?php endif; ?>
      ...
      <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-indigo-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-indigo-100 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <input x-ref="pp_nip" type="text" name="pihak_pertama[nip]" value="<?php echo e($data['pihak_pertama']['nip'] ?? ''); ?>" placeholder="NIP" 
            ...
            <div class="space-y-1.5">
              ...
              <textarea x-ref="pp_jabatan" name="pihak_pertama[jabatan]" rows="2" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-bold placeholder:text-white/40 outline-none leading-snug" required><?php echo e($data['pihak_pertama']['jabatan'] ?? ''); ?></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="bg-rose-600 rounded-[2.5rem] p-6 text-white shadow-xl shadow-rose-100 space-y-6">
          ...
          <div class="space-y-4">
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Nama Lengkap</label>
              <input type="text" name="pihak_kedua[nama]" value="<?php echo e($data['pihak_kedua']['nama'] ?? ''); ?>" placeholder="Nama Peminjam" 
            ...
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">NIP</label>
              <input type="text" name="pihak_kedua[nip]" value="<?php echo e($data['pihak_kedua']['nip'] ?? ''); ?>" placeholder="NIP / Identitas" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-mono placeholder:text-white/40 outline-none" required>
            </div>
            <div class="space-y-1.5">
              <label class="text-[8px] font-black uppercase tracking-widest opacity-60 ml-2">Jabatan</label>
              <textarea name="pihak_kedua[jabatan]" rows="2" class="w-full bg-white/10 border-none rounded-xl px-4 py-3 text-xs font-bold placeholder:text-white/40 outline-none leading-snug" required><?php echo e($data['pihak_kedua']['jabatan'] ?? ''); ?></textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-3 px-2">
        <a href="<?php echo e(route('reports.pinjam.list')); ?>" class="flex-1 py-5 bg-slate-100 text-slate-400 rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] text-center">Batal</a>
...
<?php echo $__env->make($isMobile ? 'layouts.mobile' : 'layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\SI-LARANG\resources\views/pinjam_pakai/create.blade.php ENDPATH**/ ?>